...
%% list管理API
-export([task_center/0,get_dets/0,center_tasklist_init/1,add_to_list/3,pull_task/1,write_to_list/4]).
%%回调函数
...
%%        test
%%        gen_server_file:start(task_center).
%%        gen_server_file:add_task(job1,15000).
%%        gen_server_file:add_task(job2,15000).
%%        gen_server_file:add_task(job3,15000).
%%        gen_server_file:cencel_task().
%%        gen_server_file:get_task().
%%        gen_server_file:get_dets().
%%        gen_server_file:get_task_list().
...
init([Name]) ->
  ...
  State =#state{task_center = Name,task_list = TaskList,file_source = FileSource},
